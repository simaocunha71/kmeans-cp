...
int main (int argc, char** argv){
    ...
    if(THREADS > 0){
        do{
            converged = update_clusters_ompmpi(space_x, space_y, clusters_x, clusters_y, samples_id, clusters_npoints, world_size, world_rank, status,  N_SAMPLES, K_CLUSTERS);
            iterations++;
        }while(!converged && iterations <= 20);
    }
    else {
        do{
            converged = update_clusters_mpi(space_x, space_y, clusters_x, clusters_y, samples_id, clusters_npoints, world_size, world_rank, status,  N_SAMPLES, K_CLUSTERS);
            iterations++;
        }while(!converged && iterations <= 20);
    }
